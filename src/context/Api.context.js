...
export default class API {
  ...
  async getProductbyCategory(category) {
    let a = await axios.get(
      `http://localhost:4001/product/category/` + category
    );
    return a.data;
  }
  ...
  async getProductbyPriceGreater(price) {
    let a = await axios.get(
      `http://localhost:4001/product/price/greater/` + price
    );
    return a.data;
  }
  async getProductbyPriceSmaller(price) {
    let a = await axios.get(
      `http://localhost:4001/product/price/smaller/` + price
    );
    return a.data;
  }
  async getProductbyPriceBetween(greaterthan, smallerthan) {
    let a = await axios.get(
      `http://localhost:4001/product/price/between/` +
        greaterthan +
        `/` +
        smallerthan
    );
